...
wss = new WebSocketServer({ noServer: true });
  ...
  // It intercepts the HTTP request and checks if it's a WebSocket request.
  server.on("upgrade", (request, socket, head) => {
    const { pathname } = new URL(request.url, `http://${request.headers.host}`);

    // If it's a Next.js Hot Module Replacement (dev tool) request, ignore it
    if (pathname.startsWith("/_next/")) return;

    // For all other socket requests, handle the upgrade
